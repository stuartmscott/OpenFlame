...
public class While extends ConditionalControlStatement {

    public While(ClassDeclaration declaration, int lineNum, Expression condition, StatementList statement) {
    ...
    public void emit(Generator g) {
        Label start = new Label("startWhile");
        mDeclaration.mCompiler.addLabel(start, mDeclaration.fullName(), mLineNum);
        Label end = new Label("endWhile");
        mDeclaration.mCompiler.addLabel(end, mDeclaration.fullName(), mLineNum);
        g.emitAssembly(new Jez("jump to endWhile", g.register_zero, end.mName));
        g.emitAssembly(start);
        mStatement.emit(g);
        g.emitAssembly(end);
        Register condition = mCondition.retainIntoRegister(g);
        g.emitAssembly(new Subtract("test condition", false, g.register_zero, condition, condition));//invert it (>0 becomes <0 etc)
        g.emitAssembly(new Jlz("loop to startWhile", condition, start.mName));
        g.freeExpressionRegister(condition);
    }
